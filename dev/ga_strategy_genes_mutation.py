...
class MultiMutationStrategy(MutationStrategy):
    ...
    qui mutent systématiquement un seul gène ou tous les gènes. Cette approche permet de s'adapter dynamiquement
    aux besoins spécifiques de l'algorithme. En effet, en mutant un nombre
    aléatoire de gênes, cette stratégie peut générer des solutions nécessitants des mutations à des gênes spécifiques tout en
    gardant une grande couverture des solutions possibles à travers les domaines. 

    Comme ces prédécésseurs, cette stratégie demeure générique et peut être appliquer à des problèmes
    ...
    et à la structure des problèmes à n dimensions, même lorsque ces structures ne sont pas encore connu.

    En l'appliquant sur le "shape optimizer", elle permet de trouver plus rapidement les solutions qui sont 
    proche de la solution actuel mais requirt la mutation de plus d'un paramètre, tout en gardant une couverture
    de même envergure que la stratégie de mutation de tous les gènes. Donc, si une meilleur solution est possible 
    ...
    En ce qui concerne notre problène de balistique, La stratégie de mutation qui permet de muter de 1 à n gènes à la fois 
    ...
    def mutate(self, offsprings: np.ndarray, mutation_rate: float, domains: Domains) -> None:
        def do_mutation(offspring, mutation_rate, domains):
            if self._rng.random() <= mutation_rate:
                nb_index = random.randint(1, domains.dimension)
                mutation_mask = np.zeros(domains.dimension, dtype=bool)
                mutation_indices = self._rng.choice(domains.dimension, nb_index, replace=False)
